...
export default function FilterSidebar({
  ...
  return (
    <aside className="w-56 shrink-0 rounded-2xl border border-gray-200/80 bg-white p-5 shadow-xs">
      {/* Category Section */}
      <div className="mb-6">
        ...
        <div className="space-y-2.5">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <label
                key={cat}
                className="group flex cursor-pointer items-center gap-2.5 text-xs text-gray-700 transition-colors hover:text-gray-900"
              >
                <input
                  type="checkbox"
                  checked={isChecked}
      ...
      <div className="border-t border-gray-100 pt-5">
        <div className="flex items-center justify-between">
          <span className="text-xs font-medium text-gray-700">Mandatory only</span>
          <button
            type="button"
            role="switch"
          ...
          >
            <span
              ...
              className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out ${
                mandatoryOnly ? 'translate-x-4' : 'translate-x-0'
              }`}
            />
          </button>
        </div>
